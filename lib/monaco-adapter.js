...
var MonacoAdapter = function () {
  ...
  MonacoAdapter.prototype.initEditor = function initEditor() {
    ...
    if (newEditor != this.monacoEditor) {
      ...
      if (this.monacoEditor != null) {
        ...
        // The editor can be already focused when we switch to another model, so we need to trigger the focus manually
  ...
  MonacoAdapter.prototype.getCursor = function getCursor() {
    ...
    var cursorAtEnd = position.equals(endPos)

    var startOffset = this.monacoModel.getOffsetAt(startPos);
    var endOffset = this.monacoModel.getOffsetAt(endPos);

    var start = cursorAtEnd ? endOffset : startOffset
    var selectionEnd = cursorAtEnd ? startOffset : endOffset

    /** Return cursor position */
    return new firepad.Cursor(start, selectionEnd);
  };
  ...
  MonacoAdapter.prototype.setCursor = function setCursor(cursor) {
    ...
    this.monacoEditor.setSelection(
      this.monacoNamespace.Range.fromPositions(
        this.monacoModel.getPositionAt(Math.min(position, selectionEnd)),
        this.monacoModel.getPositionAt(Math.max(position, selectionEnd))
      )
    );
    this.monacoEditor.setPosition(this.monacoModel.getPositionAt(position))
  };
  ...
  MonacoAdapter.prototype.setOtherCursor = function setOtherCursor(cursor, color, clientID, name) {
    ...
    if (otherCursor) {
      // Recreate the cursor if the color or the name has changed
      if (otherCursor.name !== name || otherCursor.color !== color) {
        this.removeOtherCursorDecoration(otherCursor)
        this.otherCursors.splice(this.otherCursors.indexOf(otherCursor), 1)
        otherCursor = null
      } else {
